...
const CGFloat kTSAlertView_LeftMargin	= 14.0;
const CGFloat kTSAlertView_TopMargin	= 16.0;
const CGFloat kTSAlertView_BottomMargin = 14.0;
...
- (UILabel*) titleLabel
{
	if ( _titleLabel == nil )
	{
		...
		_titleLabel.shadowColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.8];
...
- (UITextView *)messageTextView {
	if (_messageTextView == nil) {
		...
		_messageTextView.font = [UIFont fontWithName:@"HelveticaNeue" size:16];
...
- (void) setCancelButtonIndex:(NSInteger)buttonIndex
{
	...
	[[b titleLabel] setFont:[UIFont fontWithName:@"HelveticaNeue-Bold" size:17]];
	[[b titleLabel] setShadowOffset:CGSizeMake(0.0f, -1.0f)];
	
	UIImage *buttonBgNormal = [UIImage imageNamed:@"button_red.png"];
...
- (NSInteger)addButtonWithTitle:(NSString *)t
{
	...
	UIImage *buttonBgNormal = [UIImage imageNamed: @"button_green.png"];
	buttonBgNormal = [buttonBgNormal stretchableImageWithLeftCapWidth:buttonBgNormal.size.width / 2.0 topCapHeight:buttonBgNormal.size.height / 2.0];
	[b setBackgroundImage:buttonBgNormal forState:UIControlStateNormal];
...
- (void) show
{
	...
	TSAlertOverlayWindow* ow = [[[TSAlertOverlayWindow alloc] initWithFrame: [UIScreen mainScreen].bounds] autorelease];
	ow.alpha = 0.0;
...
- (CGSize) recalcSizeAndLayout: (BOOL) layout
{
	...
	totalHeight = _backgroundImage.size.height;
	...
	if (layout) {
		...
		if (self.title != nil) {
			self.titleLabel.frame = CGRectMake( kTSAlertView_LeftMargin, y - 1.0f, titleLabelSize.width, titleLabelSize.height);
		...
		if (self.message != nil) {
			if ( self.usesMessageTextView ) {
				...
				self.messageLabel.frame = CGRectMake(kTSAlertView_LeftMargin + 5.0f, y + 14.0, messageViewSize.width, messageViewSize.height);
		...
		if ( stacked )
		{
			...
			{
				b.frame = CGRectMake(kTSAlertView_LeftMargin + 6.0f, _backgroundImage.size.height - buttonHeight - 11.0f - kTSAlertView_BottomMargin, buttonWidth - 12.0f, buttonHeight );
				[self addSubview: b];
		...
		{
			CGFloat buttonWidth = (maxWidth - kTSAlertView_ColumnMargin) / 2.0 - 6.0f;
			CGFloat x = kTSAlertView_LeftMargin + 6.0f;
			for (UIButton *b in self.buttons) {
				b.frame = CGRectMake(x, _backgroundImage.size.height - buttonHeight - 11.0f - kTSAlertView_BottomMargin, buttonWidth, buttonHeight);
				[self addSubview: b];
...
- (CGSize)messageLabelSize {
	CGFloat maxWidth = self.width - (kTSAlertView_LeftMargin * 2) - 10.0f;
