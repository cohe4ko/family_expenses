...
-(void)deckChangeAnimation:(UIView*)destView forScr:(UIView*)srcView{
    ...
    [keyFramePath2 addLineToPoint:CGPointMake(240, 400)];
    [keyFramePath2 moveToPoint:CGPointMake(240, 400)];
	...
	theGroup.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseIn];
	theGroup2.animations = [NSArray arrayWithObjects:positionAnimation2,nil];
    [theGroup2 setDelegate:self];
...
-(void)rotate:(UIView *)withView scale:(CGFloat)scale angle:(CGFloat)angle{
    if (!withView) {
        [self notificateDelegate];
        return;
    }
    ...
    scaleAnim.toValue = [NSValue valueWithCGSize:CGSizeMake(scale,scale)];
    
    CABasicAnimation *rotateAnim = [CABasicAnimation animationWithKeyPath:@"transform.rotation"];
    [rotateAnim setToValue:[NSNumber numberWithFloat:angle]];
    
    CABasicAnimation *idenAnim = [CABasicAnimation animationWithKeyPath:@"transform.scale"];
    idenAnim.toValue = [NSValue valueWithCGSize:CGSizeMake(1.0,1.0)];
	...
	theGroup.repeatCount = 0;
	theGroup.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseIn];
	theGroup.animations = [NSArray arrayWithObjects:scaleAnim,rotateAnim,idenAnim,nil];
    [theGroup setDelegate:self];
    
    [withView.layer addAnimation:theGroup forKey:@"rotate"];
    withView.transform = CGAffineTransformRotate(withView.transform, angle);
}
...
-(void)realBounceView:(UIView*)view center:(CGFloat)center amplitude:(CGFloat)A isXAxis:(BOOL)isXAxis{
    //[UIApplication sharedApplication].keyWindow.userInteractionEnabled = NO;
    //animation parameters
    NSInteger numberOfPoints = 100.0;
    CGFloat animationDuration = 1.0;
    CGFloat animationTimeStep = animationDuration/numberOfPoints;
    
    //calculation parameters
    //A-амплитуда
    CGFloat w = view.frame.size.width;
    CGFloat k = 50.0/w;             //коэфициент упрогости
    CGFloat m = 1.0;                //масса
    CGFloat a = 0.3;                //коэфициент трения
    CGFloat nu = 0.5*a/m;           
    CGFloat omega1 = k/m;
    CGFloat omega = sqrtf(fabs(omega1-nu*nu));
    CGFloat phi = M_PI_2;           //фаза
    CGFloat timestep = 1.0;
    
    //calculations
    NSMutableArray *timesArray = [NSMutableArray array];
    UIBezierPath *bezierPath = [UIBezierPath bezierPath];
    if (!isXAxis) {
        [bezierPath moveToPoint:CGPointMake(view.layer.position.x,
                                            center+A)];
    }else {
        [bezierPath moveToPoint:CGPointMake(center+A,
                                            view.layer.position.y)];
    }
    
    for (int i=0;i<numberOfPoints;i++) {
        CGFloat t = i*timestep;
        CGFloat s = sinf(omega*t+phi);
        CGFloat e = exp(-nu*sqrtf(omega1)*t);
        CGFloat val = A*e*s;
        CGPoint p;
        if (!isXAxis) {
            p = CGPointMake(view.layer.position.x,
                            center+val);
        }else {
            p = CGPointMake(center+val,
                            view.layer.position.y);
        }
        
        [bezierPath addLineToPoint:p];
        [bezierPath moveToPoint:p];
        [timesArray addObject:[NSNumber numberWithFloat:i*animationTimeStep]];
    }
    
    CAKeyframeAnimation *bounceAnimation = [CAKeyframeAnimation animationWithKeyPath:@"position"];
    bounceAnimation.removedOnCompletion = NO;
    bounceAnimation.fillMode = kCAFillModeForwards;
    [bounceAnimation setPath:bezierPath.CGPath];
    [bounceAnimation setKeyTimes:timesArray];
