...
+ (void)initDB {
	Db *db = [[[self class] alloc] initWithName:DATABASE];
	db.path = [[self class] getDbFile];
	sharedDB = [db retain];
	[db release];
...
+ (void)createEditableCopyOfDatabaseIfNeeded:(BOOL)force {
    ...
    NSError *error;
    NSString *writableDBPath = [Db getDbFile];
...
- (void)setupDB {
	NSLog(@"createDB parent");
}

+ (Db *)getDB {
	Db *db = [[[Db alloc] initWithName:DATABASE] autorelease];
	db.path = [Db getDbFile];
	return db;
}
...
+ (NSString *)getDbFile {
	return [[Db getDbPath] stringByAppendingPathComponent:[NSString stringWithFormat:@"db_%@.sqlite", DATABASE_VERSION]];
}
