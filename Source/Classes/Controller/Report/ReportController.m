...
+ (NSMutableArray*)loadDataForReportBoxForMinDate:(NSDate*)minDate maxDate:(NSDate*)maxDate{
    NSString *sql = [NSString stringWithFormat:@"SELECT sum(t.amount) amount,  t.categoriesParentId category FROM Transactions t WHERE state = %d and time >= %d and time <= %d GROUP BY category", TransactionsStateNormal, (int)[minDate timeIntervalSince1970],(int)[maxDate timeIntervalSince1970]];
